...
import type { ContactInformationData, ContactSocialIconName } from '@/types/contact';

interface ContactInformationCardProps {
  data: ContactInformationData;
}

interface ContactSocialIconProps {
  color?: string;
  name: ContactSocialIconName;
}

function ContactSocialIcon({ color = colors.brandForeground, name }: ContactSocialIconProps) {
  const iconSize = iconSizes.compact;
  const strokeWidth = iconStrokeWidths.standard;

  if (name === 'instagram') {
    return (
      <Svg fill="none" height={iconSize} viewBox="0 0 16 16" width={iconSize}>
        <Path
          d="M11.6662 4.33325H11.6729"
          stroke={color}
          strokeLinecap="round"
        ...
        <View className="self-stretch flex-col items-start justify-start gap-3">
          ...
          <ContactSocialLinks
            items={data.socialLinks.map((link) => ({
              id: link.id,
              icon: <ContactSocialIcon name={link.icon} />,
              label: link.label,
